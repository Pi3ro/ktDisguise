package me.pi3ro.disguise.utils.menu.pagination
...
)
{
    ...
    fun handleAutoUpdate(player: Player)
    {
        ...
        retrieveInventory().thenAccept {
            ...
            } catch (e: IllegalArgumentException)
            {
                ...
                {
                    Bukkit.getScheduler().runTask(DisguisePlugin.INSTANCE) {
                        player.closeInventory()
                    }
                    player.sendMessage(translate("&cCould not set contents. Please try again"))
                }
            }
    ...
    fun updateMenu()
    {
        ...
        val inventory = Bukkit.createInventory(
            null,
            (displaySize + 9),
            translate("($currentPage/${if (maxPages == 0) 1 else maxPages}) ") + getTitle(player)
        )
